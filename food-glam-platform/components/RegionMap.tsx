...
const regions = [
  { name: 'Asian', color: 'bg-red-100 hover:bg-red-200', textColor: 'text-red-800' },
  { name: 'European', color: 'bg-blue-100 hover:bg-blue-200', textColor: 'text-blue-800' },
  { name: 'African', color: 'bg-yellow-100 hover:bg-yellow-200', textColor: 'text-yellow-800' },
  { name: 'American', color: 'bg-green-100 hover:bg-green-200', textColor: 'text-green-800' },
  { name: 'Middle Eastern', color: 'bg-purple-100 hover:bg-purple-200', textColor: 'text-purple-800' }
]
        ...
        <div className="flex gap-2 flex-wrap justify-center">
          {regions.map(region => (
            <a
              ...
              className={`${region.color} ${region.textColor} ${selectedRegion === region.name ? 'ring-2 ring-offset-2 ring-blue-500' : ''} px-3 py-1 rounded-full text-sm font-medium transition-all`}
            >
              {region.name}
            </a>
          ))}
